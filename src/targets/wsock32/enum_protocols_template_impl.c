...
static tchar *my_tcsdup(const tchar *s1)
{
    ...
    p = malloc(tcslen(s1) * sizeof(s1[0]) + 1);
    if (p != NULL)
        tcscpy(p, s1);
...
static tchar *add_protocol_name(const tchar *protocol_name)
{
    ...
    if (protocol_names_index >= protocol_names_size) {
        ...
        p = realloc(protocol_names, i * sizeof(protocol_names[0]));
        if (p == NULL) {
        ...
        LPDWORD lpdwBufferLength)
{
    ...
    protocolBuffer = malloc(bufferLength);
        ...
        LPDWORD lpdwBufferLength)
{
    ...
    PROTOCOL_INFO *protocolBuffer = lpProtocolBuffer;
    int num_protos;
